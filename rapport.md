...
## Backend
L'image backend est basée sur une image Node.js. Grâce au framework Express.js, nous avons pu coder un script qui renvoyait des données au format JSON. En l'occurrence, ce script renvoyait une liste aléatoire de 3 suggestions de repas. Comme le frontend, cette image devait signaler sa présence sur le réseau à l'attention du Reverse Proxy. Ce mécanisme a été développé grâce à un script Node.js.
